package com.example.androidmicroapps.features.imc
...
@Composable
fun ImcResultScreen(result:Float = 20f, onBackSelected: () -> Unit) {
    ...
    Scaffold(
        ...
        topBar = { TopAppBar(
            ...
            colors = TopAppBarDefaults.topAppBarColors(containerColor = BackgroundApp, titleContentColor = Color.White)
            ) }
    ) { paddingValues ->
        Column(
            modifier = Modifier
...
fun getBMIDescription(bmi: Float):String{
    return when {
        ...
        bmi < 25f -> "Tu peso es óptimo."
        bmi < 30f -> "Empieza a caminar coleguita."
        else -> "Quizás ese tercer paquete de oreos no era necesario."
    }
}
